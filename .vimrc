...
if has('autocmd')
    ...
    augroup trailingspaces
        ...
        autocmd BufWritePre *.py,*.js,*.R,*.Rmd :call <SID>StripTrailingWhitespaces()
    augroup END

    augroup ctags
        autocmd!
       " Run ctags each time a source file is saved.
       autocmd BufWritePost *.py,*.js,*.R,*.c,*.cpp,*.h call system("ctags -R --exclude=.venv --exclude=venv --exclude=test")
    augroup END
...
call minpac#add('dcbaker/vim-abook', {'type': 'opt'})
"}}}
